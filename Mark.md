...
所谓的冲突就是两个分支修改了同一份文件，从而在合并分支的时候git不知道如何合并这个文件的内容而造成的冲突。

我们首先创建一个`feat`分支，并修改`main1.txt`中的内容

![](.\pic\75.png)

然后我们切换回`main`分支，并同样修改`main.txt`中的内容

当我们想要合并分支时，就产生了冲突。如下图所示

![](.\pic\76.png)

那么，我们就需要手动的合并修改的文件来解决冲突。我们手动打开`main1.txt`

![](.\pic\77.png)

以中间的`======`为分割线，分别表示了当前分支的修改内容和`feat`分支的修改内容，我们需要手动修改这个文件后，即可进行合并。
...
rebase操作和合并的`merge`操作比较像，但又有各自的特点。`rebase`操作跟类似于像树干的嫁接和移植，它将两个分支
整合成一个完全的分支。如图所示

![](.\pic\80.png)

`rebase`两个分支，它会寻找两个分支的共同祖先，即它们产生分支的起点，并视当前的分支名称，将当前的分支到共同祖先的
一段分支节点，移植到要合并的分支的最前面。

使用`rebase`需要十分小心，因为它会改变整个分支的流程，且会影响两个分支的提交情况，但它能更好的形成一个清晰的分支开发线。


## 阶段性总结

自此，我们的git学习变告一段落，以后如果有新的学习，还会继续更新。
